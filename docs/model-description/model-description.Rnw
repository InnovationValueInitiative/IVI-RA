...
<<haq_lpvf, echo = FALSE, include = FALSE>>=
linear_pvf <- function(x, x_min, x_max, scale_min = 0, scale_max = 100){
  score <- (x - x_min) * (scale_max - scale_min) / (x_max - x_min) + scale_min
  if (x_min == x_max){
...
# linear partial value function positive
x.min <- 0
x.max <- 12
x <- seq(x.min, x.max, length.out = 100)
y <- linear_pvf(x, x.min, x.max)
dat.pos <- data.table(x = x, y = y)
p.lpfv.pos <- ggplot(dat.pos, aes(x = x, y = y)) + geom_line() +
  xlab("Performance on original scale") + ylab("Score on common scale") +
  scale_x_continuous(breaks = seq(0, 12, 3))
ggsave("figs/mcda-lpvf-pos.png", p.lpfv.pos, height = 3, width = 5)

# linear partial value function positive
x.min <- 90
x.max <- 0
x <- seq(x.max, x.min, length.out = 100)
y <- linear_pvf(x, x.min, x.max)
dat.neg <- data.table(x = x, y = y)
p.lpfv.neg <- ggplot(dat.neg, aes(x = x, y = y)) + geom_line() +
  xlab("Performance on original scale") + ylab("Score on common scale") +
  scale_x_continuous(breaks = seq(0, 90, 15))
ggsave("figs/mcda-lpvf-neg.png", p.lpfv.neg, height = 3, width = 5)
@
...
tx.snames <- iviRA::treatments[!sname %in% c("nbt", "gol"), sname]
nsims <- 1000
tx.inds <- which(iviRA::treatments$sname %in% tx.snames)
tx.names <- iviRA::treatments[sname %in% tx.snames, mname]
...
acr.tab <- acr.tab[tx.inds, ]
rownames(acr.tab) <- tx.names
...
haq.tab <- haq.tab[tx.inds]

# NICE vs IVI NMA
nice.po <- iviRA::nma.acr.naive.nice$p.overlap
pos <- which(rownames(nice.po) %in% tx.snames)
...
<<sim6-haq, echo=FALSE, results=hide>>=
treatments <- iviRA::treatments[!sname %in% c("nbt", "gol"), sname]
...
dhaq6.means <- dhaq6.sim[, .(dhaq = mean(dhaq, na.rm = TRUE)),
                           by = c("tx", "model", "sim")]
dhaq6.means.summary <- dhaq6.means[, .(mean = mean(dhaq),
                                       lower = quantile(dhaq, .025, na.rm = TRUE),
                                        upper = quantile(dhaq, .975, na.rm = TRUE)),
                                   by = c("tx", "model")]
@

<<sim6-haq-tables, echo=FALSE, results=hide>>=
# Format results
...
dhaq.fmat[dhaq.fmat == "NA" | dhaq.fmat == "NaN"] <- ""
dhaq.fest <- trimws(paste0(dhaq.fmat[, "mean"], " (", dhaq.fmat[, "lower"], ", ", 
                    dhaq.fmat[, "upper"], ")"))
dhaq.fest[dhaq.fest == "(, )"] <- "-"
...
delta <- matrix(iviRA::haq.lcgm$coef[parameter %in% c("delta2", "delta3", "delta4"), est],
                nrow = 3, byrow = TRUE)
probs <- iviRA:::mlogit_probC(x, delta)
@
...
<<haq_lcgm, echo = FALSE, results=hide>>=
# haq trajectories
t <- seq(2, 30)
xt <- 1 - (1/(1 + t))
...
<<si, echo=FALSE, results=hide>>=
set.seed(100)
pop <- sample_pop(n = 1000)
input.dat <- get_input_data(pop = pop)
parsamp <- sample_pars(n = 100, input_data = input.dat)
tx.snames <- c("cdmards", "adamtx")
tx.names <- c("cDMARDs or NBT", "tDMARDs")
...
dur.dist.l <- c("Exponential", "Weibull", "Gompertz", "Gamma", "Log-logistic", "Lognormal",
                   "Generalized gamma")
sim6 <- vector(length(dur.dist) * length(tx.snames), mode = "list")
counter <- 1
for (i in 1:length(tx.snames)){
  for(j in 1:length(dur.dist)){
    ...
    sim6[[counter]] <- sim_iviRA(tx_seqs = tx.snames[i], input_data = input.dat, pars = parsamp,
                     model_structures = mod.struct, max_months = 6)
    sim6[[counter]][, dur_dist := dur.dist.l[j]]
    sim6[[counter]][, txseq := tx.names[i]]
    counter <- counter + 1
    print(paste0("txseq ", i, ", distribution ", j))
  }
}
sim6 <- rbindlist(sim6)
sim6[, si_ever := ifelse(ttsi <= ttd, 1, 0)]
si.prop <- sim6[, .(si = mean(si_ever)), by = c("dur_dist", "txseq", "sim")]
si.prop <- si.prop[, .(si_mean = mean(si), si_lower = quantile(si, .025),
                       si_upper = quantile(si, .974)), by = c("dur_dist", "txseq")]

# tables comparing serious infection rates by drugs
si1.mtx <- si.prop[dur_dist == "Generalized gamma" & txseq == "cDMARDs or NBT",
                   si_mean]
si1.bio <- si.prop[dur_dist == "Generalized gamma" & txseq == "tDMARDs",
                   si_mean]
tab1 <- si.prop[dur_dist == "Generalized gamma", .(txseq, si_mean, si_lower, si_upper)]
...
tab2 <- si.prop[txseq == tx.names[1], .(dur_dist, si_mean)]
...
<<mortpars, echo = FALSE, results = hide>>=
...
<<survplot, echo=FALSE, results=hide>>=
set.seed(100)
theme_set(theme_bw())
...
<<treat_cost, echo = FALSE, results=hide>>=
...
for (i in 1:ncol(tab2)){
  if (names(tab2)[i] %in% c("ann_num_doses", "price_per_unit")){
...
<<resource_cost, echo = FALSE, results = hide>>=
...
<<app-lcgm-class, echo = FALSE, results = hide>>=
...
where $\lceil\cdot\rceil$ is the ceiling function and implies that products cannot be reused after opening, $dose_{amt}$ is the recommended dose of the drug, $strength_{amt}$ is the strength of the drug, $doses_{num}$ is the number of doses in a 6 month period, and $price$ is the price per unit of the treatment after discounts and rebates. For example, as shown in \autoref{tbl:treat-cost}, both the strength and the dose of adalimumab are 50 mg, so costs (before discounts and rebates) for the initial 6 month period are calculated by multiplying the number of doses (\Sexpr{ada.cost$init_num_doses}) by the WAC (\$\Sexpr{formatC(ada.cost$price_per_unit, format = "f", big.mark = ",", digits = 2)}).
...
where $weight$ is patient weight, $dose_{amt}$ is the dose per weight, and $strength_{amt}$, $price$, and $doses_{num}$ are defined in the same way as in the non-weight based scenario. To illustrate, the acquisition cost (before discounts and rebates) for infliximab after the first 6 months is calculated by multiplying each patient's weight by the dose (6 mg/kg) and dividing by the size of a vial (100 mg), and then multiplying by the number of doses (\Sexpr{formatC(ifx.cost$ann_num_doses, format = "f", digits = 2)}) and the price per unit (\$\Sexpr{formatC(ifx.cost$price_per_unit, format = "f", big.mark = ",", digits = 2)}). 
...
<<search, echo = FALSE, include = FALSE>>=
search <- list()
search$medline <- read.csv("search-medline.csv")
search$embase <- read.csv("search-embase.csv")
search$cochrane <- read.csv("search-cochrane.csv")
for (i in 1:length(search)){
  search[[i]]$results <- formatC(search[[i]]$results , format = "d", big.mark = ",")
  search[[i]]$search_terms <- gsub("\\$", "\\\\$",  search[[i]]$search_terms)
  tex <- print(xtable(search[[i]]), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = paste0("tables/", names(search)[i], "-search.txt"))
}
@
...
<<included-studies, echo = FALSE, include = FALSE>>=
included.studies <- read.csv("included-studies.csv")
tex <- print(xtable(included.studies), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/included-studies.txt")
@
...
<<eligibility-criteria, echo = FALSE, include = FALSE>>=
eli.data <- read.csv("eligibility-criteria.csv")
eli.data <- gsub("\\%", "\\\\%", as.matrix(eli.data))
tex <- print(xtable(eli.data), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/eligibility-criteria.txt")
@
...
<<tdmard-naive-definition, echo = FALSE, include = FALSE>>=
tdmardnaive.data <- read.csv("tdmard-naive-definition.csv")
tdmardnaive.data <- gsub("\\%", "\\\\%", as.matrix(tdmardnaive.data))
tex <- print(xtable(tdmardnaive.data), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/tdmard-naive-definition.txt")
@
...
<<study-characteristics, echo = FALSE, include = FALSE>>=
studychar.data <- read.csv("study-characteristics.csv")
studychar.data <- gsub("\\%", "\\\\%", as.matrix(studychar.data))
tex <- print(xtable(studychar.data), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/study-characteristics.txt")
@
...
<<patient-characteristics, echo = FALSE, include = FALSE>>=
patientchar.data <- read.csv("patient-characteristics.csv")
patientchar.data <- gsub("\\%", "\\\\%", as.matrix(patientchar.data))
tex <- print(xtable(patientchar.data), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/patient-characteristics.txt")
@
...
<<study-data, echo = FALSE, include = FALSE>>=
trial.data <- read.csv("trial-data.csv")
trial.data <- gsub("\\%", "\\\\%", as.matrix(trial.data))
tex <- print(xtable(trial.data), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/trial-data.txt")
@
...
<<excluded-studies, echo = FALSE, include = FALSE>>=
excluded.studies <- read.csv("excluded-studies.csv")
tex <- print(xtable(excluded.studies), 
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      hline.after = NULL,
      file = "tables/excluded-studies.txt")
@
