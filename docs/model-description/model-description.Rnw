...
\item \textbf{Longterm clinical outcomes}: HAQ, QALYs
...
If a CEA approach is used for value assessment, then the value of treatment is estimated using the net-monetary benefit (NMB), as described in \autoref{sec:cea}. CEA from a societal perspective would include productivity losses while analyses from a health care sector perspective would not.
