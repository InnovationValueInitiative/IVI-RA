...
The probability of stopping treatment depends on the model structure used. If \textbf{S1} or \textbf{S6} is used, then patients stop treatment when there is no ACR response or no EULAR response, and continue treatment otherwise. On the other hand, if switching depends on SDAI, CDAI, or DAS28, then we model the probability of treatment switching using the coefficients from the logistic regression equation reported in \citet{zhang2011thresholds} and analyzed using patients in the Consortium of Rheumatology Researchers of North America (CORRONA) database. The probability of switching is modeled as a function of disease activity at followup (CDAI < 10 (low), CDAI 10-22 (moderate), and CDAI $\geq$ 22 (high)). Since the model is estimated using  baseline probability of switching at 6 monts  
...
on an analysis of the CORRONA database. 
