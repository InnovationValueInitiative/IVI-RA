...
\item \textbf{Consensus-driven:} Models should reflect a range of scientifically defensible approaches. Potentially controversial elements are highlighted so that they can be debated and introduced as part of the OSVP process. 
\end{itemize}
...
\begin{enumerate}
\item Develop a flexible, open-source decision-analytic model reflecting a range of plausible modeling approaches based on clinical practice and prior scientific literature.
...
CEA is a well-established technique for health technology assessment grounded in economic theory \citep{meltzer2011theoretical}. In general, CEA can be thought of as a methodology for maximizing health or well being subject to a resource constraint \citep{garber1997economic}. The total value of a new health technology relative to a comparator is typically assessed using the incremental net monetary benefit (INMB), 
...
Each criterion is assigned points ranging from 0 to 100 by the decision maker, and weighted by dividing each criterion's points by the sum of points across all criteria. For example, if there were 3 criteria and each criterion was given a score of 50, then each criterion would receive a weight of 1/3. If, on the other hand, the three criteria were given scores of 25, 50, and 75, then they would be given weights of .167, .33, and .5, respectively. 
...
\citet{lakdawalla2017insurance} provide a general mathematical framework for incorporating the effects of medical innovation on physical and financial risk. The general idea is that innovation can lower physical risk to healthy patients who might get sick in the future. New medical technologies act like ``insurance policies'' that protect a healthy person from all or part of the costs of falling ill. And while innovation certainly increases financial risk, this increase in financial risk can be mitigated by healthcare insurance.  
...
The model simulates the health outcomes, costs, and risks associated with treatment. Depending on the model structure, simulated clinical outcomes during the initial treatment phase can include ACR response, EULAR response, DAS28, SDAI, and/or CDAI. Longterm outcomes include HAQ over time, time to treatment discontinuation, and an indicator for whether a serious infection ocurred. Total costs consist of health care sector costs (drug acquisition and administration costs, general management and monitoring costs, adverse event costs, and hospitalization costs) and productivity losses.
...
<<nma_results, cache=CACHE, echo=FALSE, include=FALSE>>=
arm.snames <- c("cdmards", "abtivmtx", "adamtx", "etnmtx", "golmtx", "ifxmtx", "tczmtx", "czpmtx", "abtscmtx", "rtxmtx", "tofmtx")
nsims <- 1000
arminds <- which(iviRA::treatments$sname %in% arm.snames)
arm.names <- iviRA::treatments[sname %in% arm.snames, mname]
...
# ACR response
acr.pars <- sample_nma_acr(nsims, m = iviRA::nma.acr.naive$mean,
                      vcov = iviRA::nma.acr.naive$vcov)
dmat <- matrix(NA, nrow = dim(acr.pars$d)[1], ncol = dim(acr.pars$d)[3])
for (i in 1:dim(acr.pars$d)[3]){
  dmat[, i] <- acr.pars$d[,,i, drop = FALSE]  %*%  t(x)
}
acr.prob <- nma_acrprob(A = acr.pars$A, z2 = acr.pars$z2, z3 = acr.pars$z3,
                        d = dmat)
acr.mean <- formatC(apply(acr.prob$overlap, c(3, 2), mean), format = "f", digits = 3)
acr.l <- formatC(apply(acr.prob$overlap, c(3, 2), quantile, .025, na.rm = TRUE), format = "f", digits = 3)
acr.u <- formatC(apply(acr.prob$overlap, c(3,2), quantile, .975, na.rm = TRUE), format = "f", digits = 3)
acr.tab <- format_table(acr.mean, acr.l, acr.u, digits = 3)
acr.tab <- acr.tab[arminds, ]
rownames(acr.tab) <- arm.names

# DAS28
das28.pars <- sample_nma_lm(nsims, m = iviRA::nma.das28.naive$mean,
                      vcov = iviRA::nma.das28.naive$vcov)
dmat <- matrix(NA, nrow = dim(das28.pars$d)[1], ncol = dim(das28.pars$d)[3])
for (i in 1:dim(das28.pars$d)[3]){
  dmat[, i] <- das28.pars$d[,,i, drop = FALSE]  %*%  t(x)
}
das28.dy <- nma_lm_dy(A = das28.pars$A, d = dmat)
das28.mean <- apply(das28.dy, 2, mean)
das28.l <- apply(das28.dy, 2, quantile, .025, na.rm = TRUE)
das28.u <- apply(das28.dy, 2, quantile, .975, na.rm = TRUE)
das28.tab <- format_table(das28.mean, das28.l, das28.u, digits = 3)
das28.tab <- das28.tab[arminds]

# HAQ
haq.pars <- sample_nma_lm(nsims, m = iviRA::nma.haq.naive$mean,
                      vcov = iviRA::nma.haq.naive$vcov)
dmat <- matrix(NA, nrow = dim(haq.pars$d)[1], ncol = dim(haq.pars$d)[3])
for (i in 1:dim(haq.pars$d)[3]){
  dmat[, i] <- haq.pars$d[,,i, drop = FALSE]  %*%  t(x)
}
haq.dy <- nma_lm_dy(A = haq.pars$A, d = dmat)
haq.mean <- apply(haq.dy, 2, mean)
haq.l <- apply(haq.dy, 2, quantile, .025, na.rm = TRUE)
haq.u <- apply(haq.dy, 2, quantile, .975, na.rm = TRUE)
haq.tab <- format_table(haq.mean, haq.l, haq.u, digits = 3)
haq.tab <- haq.tab[arminds]

# NICE vs IVI NMA
nice.po <- iviRA::nma.acr.naive.nice$p.overlap
pos <- which(rownames(nice.po) %in% arm.snames)
nice.po <- formatC(nice.po[pos, -1], format = "f", digits = 3)
nice.po <- trimws(nice.po)
nice.po[nice.po == "NA"] <- "-"
tab.acr.comp <- cbind(acr.tab[, 2:4], nice.po)

# TEX table NMA results
...
# TEX table IVI vs NICE
tex <- print(xtable(tab.acr.comp),
       include.rownames = TRUE, include.colnames = FALSE,
       only.contents = TRUE, sanitize.text.function = identity,
       file = "tables/nma-acr-nice-vs-ivi.txt")
@
...
Its important to note that treatment effects for each bDMARD were estimated relative to cDMARDs and then applied to the average response for patients using cDMARDs. A limitation of our current approach is that that the average response for patients using cDMARDs is estimated using data from the clinical trials include in the NMA, and may not reflect outcomes seen in routine practice. Future versions of the model could consider using real-world data instead of clinicial tial evidence to estimate this average response.

Given that there is limited evidence that treatment effects vary across patients in the published literature, treatment response at 6 months for a given treatment does not vary according to patient characteristics. Nonetheless, in our \R{} package, treatment effects for each simulated patient can be modeled as a function of any variables chosen by the user. Our approach to modeling treatment effect heterogeneity is described in \autoref{app:nma-statistical-models}  

Treatment effects for bDMARD experienced patients are reduced by multiplying treatment effects for bDMARD naive patients by a constant $k$. Based on evidence reported in \citet{carlson2015economic}, we assume that $k$ is uniformly distributed and ranges between .75 and .92, implying that (rounding up) the average value of $k$ is .84. In other words, reductions in DAS28 and HAQ scores for bDMARD experienced patients are, on average, 84\% of the reduction in DAS28 and HAQ scores for bDMARD naive patients, and an ACR response of 60/40/20 for bDMARD naive patients would, on average, be reduced to 50/33/16 for bDMARD experienced patients. 
...
tab <- cbind(iviRA::acr2sdai$leflunomide$mean, iviRA::acr2sdai$inception$mean,
             iviRA::acr2cdai$inception$mean, iviRA::acr2das28$inception$mean)
tab <- formatC(tab, format = "f", digits = 3)
rownames(tab) <- c("<20", "20 to <50", "50 to <70", "$\\geq$70")
tex <- print(xtable(tab), 
      include.rownames = TRUE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
...
tab <- formatC(as.matrix(iviRA::acr2haq[, .(mean, se)]))
rownames(tab) <- c("<20", "20 to <50", "50 to <70", "$\\geq$70")
tex <- print(xtable(tab), 
      include.rownames = TRUE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
...
eular2haq.f <- formatC(as.matrix(iviRA::eular2haq[, .(mean, se)]), format = "f", digits = 3)
...
<<sim6-haq, cache=CACHE, echo=FALSE, include=FALSE>>=
# arms to include
arm.snames <- c("cdmards", "abtivmtx", "adamtx", "etnmtx", "golmtx", "ifxmtx", "tczmtx", "czpmtx", "abtscmtx", "rtxmtx", "tofmtx")
arm.names <- iviRA::treatments[sname %in% arm.snames, mname]

# simulation function
sim6_summary <- function(arms, arm_names, nouter = 1000, ninner = 1000,
                         model_structure, das28_mean = 6){
  acr.mean <- matrix(NA, nrow = length(arms), ncol = 4)
  acr.o.mean <- matrix(NA, nrow = length(arms), ncol = 3)
  acr.lower <- acr.upper <- acr.mean
  acr.o.lower <- acr.o.upper <- acr.o.mean 
  eular.mean <- matrix(NA, nrow = length(arms), ncol = 3)
  eular.lower <- eular.upper <- eular.mean
  dhaq.mean <- dhaq.lower <- dhaq.upper <- rep(NA, length(arms))
  pat <- sample_pop(n = ninner, das28_mean = das28_mean)
  input.dat <- get_input_data(pop = pat)
  parsamp <- sample_pars(n = nouter, input_data = input.dat)
  for (i in 1:length(arms)){
    if (!(model_structure[1, "tx_iswitch"] == "das28-switch" & 
          arms[i] %in% c("adamtx", "tofmtx", "tox"))){
      sim6 <- sim_iviRA(arms[i], input_data = input.dat, pars = parsamp,
                        model_structures = model_structure, max_months = 6)
...
# simulate
## H1
mod.struct <- select_model_structures(tx_ihaq = "acr-haq", tx_iswitch = "acr-switch",
                                     utility_model = "wailoo")
sim6.acr2haq.naive <- sim6_summary(arm.snames, arm.names, model_structure = mod.struct)

## H2
mod.struct <- select_model_structures(tx_ihaq = "haq", tx_iswitch = "das28-switch",
                                     utility_model = "wailoo")
sim6.haq.naive <- sim6_summary(arm.snames, arm.names,
                                model_structure = mod.struct, das28_mean = 0)

## H3
mod.struct <- select_model_structures(tx_ihaq = "acr-eular-haq", 
                                     tx_iswitch = "acr-eular-switch",
                                     utility_model = "wailoo")
sim6.acr2eular2haq.naive <- sim6_summary(arm.snames, arm.names, 
                                         model_structure = mod.struct)
@

<<sim6-haq-tables, cache=CACHE, echo=FALSE, include=FALSE>>=
summary_table <- function(x, digits = 2){
  # table output
  ...
  ftable.mean <- trimws(formatC(table.mean, format = "f", digits = digits))
  ftable.lower <- trimws(formatC(table.lower, format = "f", digits = digits))
  ftable.upper <- trimws(formatC(table.upper, format = "f", digits = digits))
  ftable.mean[ftable.mean == "NA"] <- ""
  ftable.lower[ftable.lower == "NA"] <- ""
  ftable.upper[ftable.upper == "NA"] <- ""
  ftable <- paste0(ftable.mean, " (", ftable.lower, ", ", ftable.upper, ")")
  ftable[ftable == " (, )"] <- "-"
  ftable <- matrix(ftable, nrow = nrow(ftable.mean), ncol = ncol(ftable.mean))
  ftable <- trimws(ftable)
  cn <- c("acrl20", "acr20-50", "acr50-70", "acr70+", "acr20", "acr50", "acr70",
                           "eular_nr", "eular_mr", "eular_gr", "dhaq")
  colnames(ftable) <- cn
...
# H1 - H3
tab.acr2haq.naive <- summary_table(sim6.acr2haq.naive)
tab.haq.naive <- summary_table(sim6.haq.naive)
tab.acr2eular2haq.naive <- summary_table(sim6.acr2eular2haq.naive)

# TEX table
tab.dhaq.naive <- cbind(tab.acr2haq.naive[, "dhaq"], tab.haq.naive[, "dhaq"], tab.acr2eular2haq.naive[, "dhaq"]) 
rownames(tab.dhaq.naive) <- arm.names
tex <- print(xtable(tab.dhaq.naive),
      include.rownames = TRUE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
...
tab1 <- iviRA::haq.lprog$tx[sname == "cdmards", .(est, lower, upper)]
...
tab2 <- iviRA::haq.lprog$diff.age[, .(est, lower, upper)] 
...
delta <- matrix(iviRA::haq.lcgm$coef[parameter %in% c("delta2", "delta3", "delta4"), est],
                nrow = 3, byrow = TRUE)
...
beta1 <- iviRA::haq.lcgm$coef[parameter == "beta1", est]
beta2 <- iviRA::haq.lcgm$coef[parameter == "beta2", est]
beta3 <- iviRA::haq.lcgm$coef[parameter == "beta3", est]
beta4 <- iviRA::haq.lcgm$coef[parameter == "beta4", est]
haq.lcgm1 <- haq_lcgm(beta1)
haq.lcgm2 <- haq_lcgm(beta2)
haq.lcgm3 <- haq_lcgm(beta3)
haq.lcgm4 <- haq_lcgm(beta4)
haq.lprog <- iviRA::haq.lprog$tx[sname == "cdmards", est] 
...
p <- ggplot(p.dat, aes(x = t, y = value, col = factor(type))) + geom_line() + 
  geom_point(size = .75) +
...
<<si, echo=FALSE, include=FALSE, cache=CACHE>>=
set.seed(100)
pop <- sample_pop(n = 1000)
input.dat <- get_input_data(pop = pop)
parsamp <- sample_pars(n = 1000, input_data = input.dat)
...
dur.dist.l <- c("Exponential", "Weibull", "Gompertz", "Gamma", "Log-logistic", "Lognormal",
                   "Generalized gamma")
sim6 <- vector(length(dur.dist) * length(arm.snames), mode = "list")
counter <- 1
for (i in 1:length(arm.snames)){
  for(j in 1:length(dur.dist)){
    mod.struct <- select_model_structures(tx_ihaq = "acr-haq",
                                     tx_iswitch = "acr-switch",
                                     cdmards_haq_model = "lcgm",
                                     ttd_dist = dur.dist[i],
                                     utility_model = "wailoo")
    sim6[[counter]] <- sim_iviRA(arms = arm.snames[i], input_data = input.dat, pars = parsamp,
                     model_structures = mod.struct, max_months = 6)
    sim6[[counter]][, dur_dist := dur.dist.l[j]]
    sim6[[counter]][, arm := arm.names[i]]
    counter <- counter + 1
    print(paste0("arm ", i, ", distribution ", j))
  }
}
sim6 <- rbindlist(sim6)
...
<<utility, echo=FALSE, include=FALSE, cache=CACHE>>=
n <- 1000
pat <- sample_pop(n = n)
haq = seq(0, 3, .1)
simhaq <- data.table(id = 1, sim = rep(seq(1, n), each = length(haq)), haq = rep(haq, n), age = 55)
input.dat <- get_input_data(pat)
parsamp <- sample_pars(n = n, input_data = input.dat)
util.mix <- sim_utility_mixture(simhaq, male = pat[, "male"], pars = parsamp$utility.mixture)
util.wailoo <- sim_utility_wailoo(simhaq, haq0 = pat[, "haq0"], male = pat[, "male"],
                                  prev_dmards = pat[, "prev_dmards"], 
...
tab1 <- iviRA::mort.or[, .(logor, logor_lower, logor_upper)]
tab1 <- formatC(as.matrix(tab1), format = "f", digits = 3)
...
tab2 <- iviRA::mort.hr.haqdif[, .(loghr, loghr_lower, loghr_upper)]
tab2 <- formatC(as.matrix(tab2), format = "f", digits = 3)
...
<<survplot, echo=FALSE, include=FALSE, cache=CACHE>>=
set.seed(100)
theme_set(theme_bw())
...
surv.mean.lhaq <- mean(surv[gender == "Female" & lab == "HAQ increase of 0.03 per year", surv])
surv.mean.nora.m <- mean(surv[gender == "Male" & lab == "No RA", surv])
surv.mean.chaq.m <- mean(surv[gender == "Male" & lab == "Constant HAQ", surv])
surv.mean.lhaq.m <- mean(surv[gender == "Male" & lab == "HAQ increase of 0.03 per year", surv])
...
tab = copy(iviRA::tx.cost$cost)
tab[, init_cost := init_dose_val/strength_val * init_num_doses * price_per_unit + init_num_doses * infusion_cost]
tab[, ann_cost := ann_dose_val/strength_val * ann_num_doses * price_per_unit + ann_num_doses * infusion_cost]

# cost for weight based dosing
wt <- sample_pop(1000)[, "weight"]
male.prop <- formals(sample_pop)$male_prop * 100
wtmale <- formals(sample_pop)$wtmale
wtfemale <- formals(sample_pop)$wtfemale
ifx <- tab[sname == "ifx"]
init.cost.ifx <- mean(ceiling(wt * ifx$init_dose_val / ifx$strength_val) * ifx$init_num_doses * ifx$price_per_unit + ifx$init_num_doses * ifx$infusion_cost)
ann.cost.ifx <- mean(ceiling(wt * ifx$ann_dose_val / ifx$strength_val) * ifx$ann_num_doses * ifx$price_per_unit + ifx$ann_num_doses * ifx$infusion_cost) 
tab[sname == "ifx", init_cost := init.cost.ifx]
tab[sname == "ifx", ann_cost := ann.cost.ifx]

# formatting
tab1 <- tab[, .(name, dosage, strength_dosage_form)]
tab2 <- tab[, .(init_num_doses, ann_num_doses, price_per_unit, infusion_cost, 
                              init_cost, ann_cost)]
tab2[, ann_num_doses := round(ann_num_doses, 2)]
tab2f <- matrix(NA, nrow = nrow(tab2), ncol = ncol(tab2))
for (i in 1:ncol(tab2)){
  if (names(tab2)[i] %in% c("ann_num_doses", "wac_per_unit")){
      tab2f[, i] <- formatC(tab2[[i]], format = "f", digits = 2, big.mark = ",")
  } else{
      tab2f[, i] <- formatC(tab2[[i]], format = "d", big.mark = ",") 
  }
}
tab2f[, 2] <- sub(".00", "", tab2f[, 2])
...
An overview of drug acquisition and administration costs is presented in \autoref{tbl:treat-cost}. Costs are a function of dose and frequency of administration, strength and dosage form, price, and infusion costs. Since infliximab dosing depend on patient weight, the costs for infliximab reported in the table average over a patient population that is \Sexpr{male.prop}\% male. The prices in the table are based on the wholesale acquisition cost (WAC) and do not include discounts or rebates so they may be higher than actual drug costs. In the simulation, a unique discount can be used for each drug; currently the discount is assumed to range from 20\% to 30\%. The methodology used to calculate drug acquisition and administration costs is described in more detail in \autoref{app:treat-cost}. 
...
tab1 <- data.table(mean = iviRA::hosp.cost$days_mean,
                   se = iviRA::hosp.cost$days_se)
...
tab2 <-data.table(mean = iviRA::hosp.cost$cost_pday_mean,
                   se = iviRA::hosp.cost$cost_pday_se)
...
tab3 <- iviRA::mgmt.cost[, .(est, lower, upper)]
...
# productivity loss
tab4 <- data.table(mean = iviRA::prod.loss$est,
                   se = iviRA::prod.loss$se)
...
npats <- 1000
pop <- sample_pop(n = npats, type = "heterog")
tab <- cbind(apply(pop, 2, mean, na.rm = TRUE), 
            apply(pop, 2, quantile, .025, na.rm = TRUE),
            apply(pop, 2, quantile, .975, na.rm = TRUE))
rownames(tab) <- c("Age", "Male", "Weight (kg)", "Previous DMARDs",
                   "DAS28", "SDAI", "CDAI", "HAQ")
...
We used this sampling procedure to simulate \Sexpr{formatC(npats, format = "d", big.mark = ",")} patients. Summary statistics from a simulated patient cohort of size \Sexpr{formatC(npats, format = "d", big.mark = ",")} are shown in \autoref{tbl:pats-samp}.
...
coefs = copy(iviRA::haq.lcgm)$coef
coefs[, lower := est - qnorm(.975) * se]
coefs[, upper := est + qnorm(.975) * se]
...
coefs.c <- coefs.c[, .(parameter, est, lower, upper)]
...
coefs.t <- coefs.t[, .(parameter, est, se)]
...
tab <- data.frame(coef = iviRA::utility.wailoo$est,
                  se = iviRA::utility.wailoo$se)
...
<<ada_cost, echo = FALSE, include = FALSE>>=
ada.cost <- iviRA::tx.cost$cost[sname == "ada"]
ifx.cost <- iviRA::tx.cost$cost[sname == "ifx"]
@
...
where $\lceil\cdot\rceil$ is the ceiling function and implies that products cannot be reused after opening, $dose_{amt}$ is the recommended dose of the drug, $strength_{amt}$ is the strength of the drug, $doses_{num}$ is the number of doses in a 6 month period, and $WAC$ is the WAC. For example, as shown in \autoref{tbl:treat-cost}, both the strength and the dose of adalimumab are 50 mg, so costs for the initial 6 month period are calculated by multiplying the number of doses (\Sexpr{ada.cost$init_num_doses}) by the WAC (\$\Sexpr{formatC(ada.cost$wac_per_unit, format = "f", big.mark = ",", digits = 2)}).
...
where $weight$ is patient weight, $dose_{amt}$ is the dose per weight, and $strength_{amt}$, $WAC$, and $doses_{num}$ are defined in the same way as in the non-weight based scenario. To illustrate, the acquisition cost for infliximab after the first 6 months is calculated by multiplying each patient's weight by the dose (6 mg/kg) and dividing by the size of a vial (100 mg), and then multiplying by the number of doses (\Sexpr{formatC(ifx.cost$ann_num_doses, format = "f", digits = 2)}) and the WAC (\$\Sexpr{formatC(ifx.cost$wac_per_unit, format = "f", big.mark = ",", digits = 2)}).  
