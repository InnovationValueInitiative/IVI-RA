...
# simulation function
sim6_summary <- function(arms, arm_names, nouter = 1000, ninner = 1000, treat_hist = "naive",
  ...
  input.dat <- get_input_data(patdata = pat, model_structure = model_structure)
  ...
  for (i in 1:length(arms)){
    if (!(model_structure[1, "tx_iswitch"] == "das28-switch" & 
      ...
      sim6 <- sim_iviRA(arms[i], input_data = input.dat, pars = parsamp,
                        model_structure = model_structure, max_months = 6)
...
sim6.acr2haq.naive <- sim6_summary(arm.snames, arm.names, treat_hist = "naive", 
                                   model_structure = mod.struct)

## H2
mod.struct <- select_model_structures(tx_ihaq = "haq", tx_iswitch = "das28-switch",
                                     utility_model = "wailoo")
sim6.haq.naive <- sim6_summary(arm.snames, arm.names, treat_hist = "naive",
...
mod.struct <- select_model_structures(tx_ihaq = "acr-eular-haq", 
                                     ...
                                     utility_model = "wailoo")
sim6.acr2eular2haq.naive <- sim6_summary(arm.snames, arm.names, treat_hist = "naive", 
                                         model_structure = mod.struct)
@
...
for (i in 1:length(arm.snames)){
  for(j in 1:length(dur.dist)){
    ...
    sim6[[counter]] <- sim_iviRA(arms = arm.snames[i], input_data = input.dat, pars = parsamp,
                     model_structure = mod.struct, max_months = 6)
...
tab = copy(iviRA::treat.cost$cost)
...
ada.cost <- iviRA::treat.cost$cost[sname == "ada"]
ifx.cost <- iviRA::treat.cost$cost[sname == "ifx"]
@
