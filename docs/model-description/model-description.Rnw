...
The probability of switching treatment during the initial treatment phase is modeled using 6 possible structures labeled \textbf{S1-S6}.
...
\subsubsection{Treatment duration in the US}\label{ttd-overall}
...
\section{Validation}
We aim to validate the model using the five types of validation described by \citet{eddy2012model}. Currently, we are able to use the first three validation types. First, we have checked the model for face validity by ensuring that simulated outcomes are consistent with current science and evidence. Second, we performed unit tests to verify that the individual units of code that are used to simulate the model return expected results. Third, we compared simulated results for key outcomes such as mortality, HAQ over time, and time to treatment discontinaution with real-world data and our underlying paramter values. In particular, we ran the model online under various scenarios using our R Shiny web application and checked the simulated outcomes. 

In the future, we plan to use both external validation and preditive validation to help fine tune our model. External validation will be performed by comparing outcomes simulated using our model to real-world outcomes and predictive validty will involve using our model to forecast future events andcomparing our forecasted outcomes to the observed outcomes. 

\section{Limitations and areas for improvement}
The IVI-RA model is an open-source collaborative model that is meant to be updated and improved over time. We believe that there are number of potential areas for improvement.

\begin{itemize}
\item \textbf{A new NMA for adverse events:}
\item \textbf{Adverse events other than serious infections:}
\item \textbf{Time to treatment discontinuation:}
\item \textbf{Patient preferences:} 
...
\item \textbf{Treatment effect modifiers:}
\item \textbf{Treamtment effects after treatment failure:}
\item \textbf{A LCGM for the progression of bDMARDs over time:}
\item \textbf{New data to specify the population:}
\item \textbf{Estimating the rebound effect:}
\end{itemize}
