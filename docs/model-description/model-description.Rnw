...
The IVI026 family of models is available as an R package, but the
majority of the code for the IPS is written in C++ so that the models
can be simulated in a reasonable time frame. Documentation is available
online at \url{https://www.google.com}.
...
<<si, echo = FALSE, include = FALSE, cache = TRUE, cache.lazy = FALSE>>=
# set.seed(100)
