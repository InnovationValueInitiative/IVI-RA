...
ir_table <- function(x, digits = 2){
  # table output
  table.mean <- cbind(x$acr.mean, x$acr.o.mean, x$eular.mean, x$hc.mean)
  table.lower <- cbind(x$acr.lower, x$acr.o.lower, x$eular.lower, x$hc.lower)
  table.upper <- cbind(x$acr.upper, x$acr.o.upper, x$eular.upper, x$hc.upper)
...
# ACR response
tab.acr <- cbind(tab.naive[, c("acr20", "acr50", "acr70")],
                 tab.exp[, c("acr20", "acr50", "acr70")])
tex <- print(xtable(tab.acr),
      include.rownames = TRUE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      file = "tables/nma-acr-response.txt")

# ACR response IVI NMA vs. NICE NMA
...
tab.acr.comp <- cbind(tab.naive[, c("acr20", "acr50", "acr70")], nice.po)
tab.acr.comp <- trimws(tab.acr.comp)
tab.acr.comp[tab.acr.comp == "NA"] <- "-"
rownames(tab.acr.comp) <- arm.snames
...
\begin{center}
\begin{threeparttable}
\caption{NMA estimates of ACR response} \label{tbl:nma-acr}
\scriptsize
\begin{tabular}{lrrrrrr}
\hline
\multicolumn{1}{c}{} & \multicolumn{3}{c}{Biologic naive} & \multicolumn{3}{c}{Biologic experienced}\\
...
\hline
\ExpandableInput{tables/nma-acr-response.txt}
\hline
\end{tabular}
\scriptsize
...
Table ? compares the impact of treatment on HAQ using model structures \textbf{H1-H3}.
...
\caption{A comparison of predicted yearly changes in HAQ between a latent class growth model and constant linear progression }\label{fig:lcgm-vs-linear}
\end{figure}
