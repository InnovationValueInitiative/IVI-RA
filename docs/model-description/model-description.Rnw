...
\begin{minipage}{\linewidth}
\footnotesize
Notes: ACR: American College of Rheumatology; EULAR: European League Against Rheumatism; HAQ: Health Assessment Questionnaire: AEs: adverse events; QALYs: quality-adjusted life-years; WTP: willingness to pay. Disease activity refers to the Disease Activity Score with 28-joint counts (DAS28) or the Simplified Disease Activity Index (SDAI).
\end{minipage}
\end{figure}
...
# NICE vs IVI NMA
nice.po <- therapy.pars$nice.nma.acr.naive$p.overlap
...
<<sim6-haq, cache=TRUE, echo=FALSE, include=FALSE>>=
## arms to include
arm.snames <- c("cdmards", "abtivmtx", "adamtx", "etnmtx", "golmtx", "ifxmtx", "tczmtx", "czpmtx",
                          "abtscmtx", "rtxmtx", "tofmtx")
...
## simulation function
sim6_summary <- function(arminds, arm_names, nouter = 1000, ninner = 1000, treat_hist = "naive",
  ...
  for (i in 1:length(arminds)){
    sim6 <- sim_haq(arminds[[i]], input_data = input.dat, pars = parsamp, max_months = 6,
                    itreat_haq = itreat_haq, itreat_switch = itreat_switch)
...
## simulate
# H1 - H3
sim6.acr2haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", itreat_haq = "acr-haq",
                                   itreat_switch = "acr-switch")
sim6.haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", itreat_haq = "haq",
                                   itreat_switch = "das28-switch")
sim6.acr2eular2haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", itreat_haq = "acr-eular-haq",
                                   itreat_switch = "acr-eular-switch")
@

<<sim6-haq-tables, cache=TRUE, echo=FALSE, include=FALSE>>=
...
An important question related to the sensitivity of results to the model specification is whether the probability of serious infections depends on the distribution used to model time to treatment discontinuation. \autoref{tbl:si-prop-bydist} consequently compares serious infection probabilities by the distribution used to simulate treatment duration. There are very small differences across distributions, suggesting that the treatment duration distribution has almost no impact on the probability of serious infections.  
...
\begin{enumerate}
\item \textbf{If} \textbf{S1-S6} leads to a treatment switch or if the sampled time to serious infection occurs during cycle 0 (i.e., $T_{si} = 0$), \textbf{then} stop treatment.
...
ada.cost <- therapy.pars$cost[sname == "ada"]
ifx.cost <- therapy.pars$cost[sname == "ifx"]
@
...
where $\lceil\rceil$ is the ceiling function and implies that products cannot be reused after opening, $dose_{amt}$ is the recommended dose of the drug, $strength_{amt}$ is the strength of the drug, $doses_{num}$ is the number of doses in a 6 month period, and $WAC$ is the WAC. For example, as shown in \autoref{tbl:treat-cost}, both the strength and the dose are 50 mg, so costs are calculated by multiplying the number of doses (\Sexpr{ada.cost$init_num_doses}) by the WAC (\$\Sexpr{formatC(ada.cost$wac_per_unit, format = "f", big.mark = ",", digits = 2)}).
