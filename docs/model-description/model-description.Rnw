...
tab <- treat.switch[, .(variable, logor, logor_lower, logor_upper)]
p.low <- 1/(1 + exp(-(tab[variable == "intercept", logor])))
p.moderate <- 1/(1 + exp(-(tab[variable == "intercept", logor] +
                             tab[variable == "da_moderate", logor])))
tab <- tab[, .(logor, logor_lower, logor_upper)]
tab <- rbind(tab[1],
             data.table(logor = 0, logor_lower = 0, logor_upper = 0),
             tab[2:3])
tab <- cbind(c("Intercept", "CDAI <10 (low)", "CDAI 10-22 (moderate)", "CDAI $\\geq 22$"), 
...
The probability of stopping treatment depends on the model structure used. If \textbf{S1} or \textbf{S6} is used, then patients stop treatment when there is no ACR response or no EULAR response, and continue treatment otherwise. If switching depends on SDAI, CDAI, or DAS28, then we model the probability of treatment switching and adjust it according to the level of disease activity. We estimated a baseline probability of treatment switching at 6 months using a Kaplan-Meier curve from an analysis from the Consortium of Rheumatology Researchers of North America (CORRONA) database (see \autoref{sec:ttd-corrona} for more details). The patient population tended to have moderate disease activity (mean CDAI = 16.0) and at 6 months, \Sexpr{p.moderate*100}\% of patients were found to discontinue treatment, which is consistent with ranges from the literature \citep{souto2015rate}. 

Differences in discontinuation by disease activity level were estimated from the logistic regression reported in \citet{zhang2011thresholds} (which also analyzed  patients from  the CORRONA database). Disease activity was measured using the CDAI (<10 (low), 10-22 (moderate), $\geq$ 22 (high)). One issue is that the probability of discontinuation estimated from the CORRONA database (\Sexpr{p.moderate*100}\%) reflects outcomes for patients that, on average, have moderate disease activity, while \citet{zhang2011thresholds} use low disease activity as the reference group.  We therefore adjusted the probability of discontinuation downwards to reflect low diseasee activity using the inverse of the odds ratio for moderate disease activity from \citet{zhang2011thresholds} (see \autoref{app:odds-ratio-prob} for an explanation of how we adjust probabilities using odds ratios). This resulted in a probability of discontinuation of \Sexpr{formatC(p.low*100, format = "f", digits = 2)}\% at 6 months. 

We used the probability of discontinuation for patients with low disease activity and the odds ratios from \citet{zhang2011thresholds} to form a logistic regression equation. The intercept was set so that the probability of discontinuation for patients with low disease activity was \Sexpr{formatC(p.low*100, format = "f", digits = 2)}\% and the odds ratios from \citet{zhang2011thresholds} were converted to log odds ratios. The coefficients are shown in \autoref{tbl:logistic-treat-switch}.

\begin{table}[!ht]
\begin{center}
\begin{threeparttable}
\caption{Coefficients from a logistic regression of the probability of switching treatment at 6 months} \label{tbl:logistic-treat-switch}
...
In our simulation, we use the absolute level of disease activity at 6 months (not at baseline) to make predictions. We define low, moderate, and high disease activity when using DAS28 and SDAI as in \citet{anderson2012rheumatoid}. 
...
\autoref{tbl:initial-haq} compares the impact of treatment on HAQ using model structures \textbf{H1-H3}.
...
tex <- print(xtable(tab),
      include.rownames = TRUE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
      file = "tables/initial_response.txt")
@
...
\hline
\ExpandableInput{tables/initial_response.txt}
\hline
\end{tabular}
\tiny
...
\section{Mathematical methods}\label{app:math}
...
\subsection{Mapping ACR response to changes in disease activity}\label{app-acr-da}
...
\end{align}
where $\delta_s$ is the vector of regression coefficients associated with class $s$ and $w_i$ is the corresponding design matrix for individual $i$. The variables included in $w_i$ are age, gender, baseline DAS28, symptom duration, rheumatoid factor, ACR criteria, and socioeconomic status. 
...
\begin{align}
p_m = \frac{1}{1 + \exp{\left[-(\text{logit}(q_x) + HAQ \cdot \log(OR))\right]}}
\end{align}
...
Utility was simulated in a two stages using the mixture model estimated by \citet{alava2013relationship}. In the first stage, we sampled pain for a given individual in a particular model cycle based on the HAQ score. In the second stage, we simulated utility as a function of HAQ, pain and age/sex.
...
\subsection{Sampling parameters for PSA} \label{sample-pars-psa}
% <<samp-pars, echo = FALSE, include = FALSE, cache = TRUE>>=
% set.seed(100)
% pat <- sample_pats(n = 100)
% parsamp <- sample_pars(n = 1000)
% parsamp.table <- par_table(parsamp, pat)
% for (i in c("Posterior mean", "Posterior SD", "Posterior 2.5%", "Posterior 97.5%")){
%   parsamp.table[, (i) := formatC(get(i), format = "f", digits = 4, big.mark = ",")]
% }
% parsamp.table[, Parameter := gsub("\\^2", "$\\^2$", Parameter)]
% tex <- print(xtable(parsamp.table), 
%       include.rownames = FALSE, include.colnames = FALSE,
%       only.contents = TRUE, sanitize.text.function = identity,
%       file = "tables/par-samp.txt")
% @
% 
% \begin{center}
% \scriptsize
% \renewcommand*{\arraystretch}{1.6}
% \begin{longtable}{p{1.2in}p{1in}ccccp{.75in}l}
%   \caption{Posterior distribution of input parameters in PSA}\\
%   \hline
% \multicolumn{1}{c}{Group} & \multicolumn{1}{c}{Parameter} & \multicolumn{1}{c}{Mean} & \multicolumn{1}{c}{SD} & \multicolumn{1}{c}{2.5\%} & \multicolumn{1}{c}{97.5\%} & \multicolumn{1}{c}{Distribution} & \multicolumn{1}{c}{Source} \\
%   \hline 
% \endfirsthead
%   \caption[]{Posterior distribution of input parameters in PSA}\\
%   \hline
...
% \endfoot
% \endlastfoot
% \ExpandableInput{tables/par-samp.txt}
% \end{longtable}
% \end{center}
