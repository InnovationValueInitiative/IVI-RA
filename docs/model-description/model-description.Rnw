...
\begin{figure}
\centering
\includegraphics[max size={.8\textwidth}{.8\textheight}]{figs/surv.png}
...
\subsection{Logistic regression} \label{app:logistic-regression}
...
\subsection{Simulating utility with a mixture model}\label{app:sim-utility-mixture}
