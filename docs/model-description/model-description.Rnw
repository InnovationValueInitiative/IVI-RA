...
sim6_summary <- function(arms, arm_names, nouter = 1000, ninner = 1000,
  ...
  input.dat <- get_input_data(patdata = pat)
...
npats <- 1000
pats <- sample_pop(n = npats, type = "heterog")
pats <- pats[, -c(which(colnames(pats) %in% c("mrs")))]
tab <- cbind(apply(pats, 2, mean, na.rm = TRUE), 
            apply(pats, 2, quantile, .025, na.rm = TRUE),
            apply(pats, 2, quantile, .975, na.rm = TRUE))
