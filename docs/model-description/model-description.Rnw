...
The effects of treatment on ACR response, DAS28, and HAQ at 6 months are estimated using a using a Bayesian network meta-analysis (NMA) of published randomized controlled trials (RCTs) (see \autoref{sec:tech-nma-initial}). 
