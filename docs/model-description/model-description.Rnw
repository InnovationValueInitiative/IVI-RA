...
sim6_summary <- function(arms, arm_names, nouter = 1000, ninner = 1000,
  ...
  pat <- sample_pats(n = ninner, das28_mean = das28_mean)
...
where $\rm{weight}$ is patient weight, $dose_{amt}$ is the dose per weight, and $strength_{amt}$, $WAC$, and $doses_{num}$ are defined in the same way as in the non-weight based scenario. To illustrate, the acquisition cost for infliximab after the first 6 months is calculated by multiplying each patient's weight by the dose (6 mg/kg) and dividing by the size of a vial (100 mg), and then multiplying by the number of doses (\Sexpr{formatC(ifx.cost$ann_num_doses, format = "f", digits = 2)}) and the WAC (\$\Sexpr{formatC(ifx.cost$wac_per_unit, format = "f", big.mark = ",", digits = 2)}).  
