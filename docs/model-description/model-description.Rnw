...
sim6_summary <- function(arminds, arm_names, nouter = 1000, ninner = 1000, treat_hist = "naive",
                         itreat_haq, itreat_switch, das28_mean = 6){
  ...
  input.dat <- input_data(patdata = pat, itreat_switch = itreat_switch)
  parsamp <- sample_pars(n = nouter, treat_hist = treat_hist)
  for (i in 1:length(arminds)){
    if (!(itreat_switch == "das28-switch" & arm.snames[i] %in% c("adamtx", "tofmtx", "tox"))){
...
## simulate
# H1 - H3
sim6.acr2haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", 
                                    itreat_haq = "acr-haq",
                                   itreat_switch = "acr-switch")
sim6.haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", itreat_haq = "haq",
                                   itreat_switch = "das28-switch", das28_mean = 0)
sim6.acr2eular2haq.naive <- sim6_summary(arminds, arm.names, treat_hist = "naive", 
                                         itreat_haq = "acr-eular-haq",
                                   itreat_switch = "acr-eular-switch")
@

<<sim6-haq-tables, cache=FALSE, echo=FALSE, include=FALSE>>=
