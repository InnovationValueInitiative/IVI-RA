...
\autoref{tbl:sim-haq-change} compares the impact of treatment on HAQ when using \textbf{H1-H3}. Results were estimated by simulating $1,000$ patients for 6 months and randomly sampling $1,000$ parameter sets. For each randomly sampled parameter set, we calculated the average decrease in HAQ at 6 months across the $1,000$ patients. Estimates reported in the table are the mean and 95\% credible of the mean decrease in HAQ at 6 months. To maintain consistency across \textbf{H1-H3}, we did not allow HAQ scores for patients who might have otherwise switched treatments accoring to \textbf{S1-S6} to rebound back to their baseline levels (i.e., levels at the start of the simulation) at the end of the 6 month period.   

<<sim6-haq, cache=CACHE, echo=FALSE, include=FALSE>>=
treatments <- c("cdmards", "abtivmtx", "adamtx", "etnmtx", "golmtx", "ifxmtx", "tczmtx", "czpmtx", "abtscmtx", "rtxmtx", "tofmtx")
pop <- sample_pop(n = 1000)
input.dat <- get_input_data(pop = pop)
parsamp <- sample_pars(n = 1000, input_data = input.dat)
dhaq6.sim <- sim_dhaq6(treatments = treatments, input_data = input.dat,
                       pars = parsamp, hist = "naive")
dhaq6.means <- dhaq6.sim[, .(dhaq = mean(dhaq)),
                           by = c("tx", "model", "sim")]
dhaq6.means.summary <- dhaq6.means[, .(mean = mean(dhaq),
                                       lower = quantile(dhaq, .025),
                                        upper = quantile(dhaq, .975)),
                                   by = c("tx", "model")]
@

<<sim6-haq-tables, cache=CACHE, echo=FALSE, include=FALSE>>=
# Format results
dhaq6.means.summary[, tx_sname := iviRA::treatments[tx, sname]]
dhaq.mat <- as.matrix(dhaq6.means.summary[, .(lower, mean, upper)])
dhaq.fmat <- trimws(formatC(dhaq.mat, format = "f", digits = 2))
dhaq.fest <- paste0(dhaq.fmat[, "mean"], " (", dhaq.fmat[, "lower"], ", ", 
                    dhaq.fmat[, "upper"], ")")
dhaq6.dt <- data.table(tx_name = dhaq6.means.summary$tx_sname,
                     model = as.character(dhaq6.means.summary$model),
                     est = dhaq.fest)
dhaq6.dt[, model := factor(model, levels = c("acr-haq", "acr-eular-haq", "haq"))]
dhaq6.table <- dcast(dhaq6.dt, tx_name ~ model, value.var = "est")
dhaq6.table <- dhaq6.table[match(treatments, tx_name)]
mname <- iviRA::treatments[match(dhaq6.table$tx_name, iviRA::treatments$sname), mname]
dhaq6.table[, tx_name := mname]

# TEX table
tex <- print(xtable(dhaq6.table),
      include.rownames = FALSE, include.colnames = FALSE,
      only.contents = TRUE, sanitize.text.function = identity,
