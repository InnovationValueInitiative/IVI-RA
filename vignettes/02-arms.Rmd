...
A dataset of available treatments comes preloaded with the `iviRA` package. The `name`, `mname`, and `sname` columns are long-form, medium-form, and short-form names, respectively. The column `biologic` is an indicator equal to 1 if the drug is a biologic and 0 otherwise. 

```{r }
print(iviRA::therapy.pars$info)
```
...
For example, suppose that we want to simulate a treatment sequence of three biologics: adalimumab + methotrexate, etanercept + methotrexate, and infliximab + methotrexate. They should be specified as a vector with elements matching the `sname` in `therapy.pars$info`. For comparison purposes, we might want to also simulate outcomes for a non-biologic treatment sequence of only cDMARDs.


```{r }
arm1 <- c("adamtx", "etnmtx", "ifxmtx")
arm2 <- c("cdmards")
```

In some cases, it might also be useful to allow therapies to differ accorss patients. This could be useful wheh, for example, comparing treatment strategies that tailor treatment to individuals against one size fits all strategies. This can be done by creating a matrix of treatment sequences where the number of rows is equal to the number of patients in the cohort. Below we consider a scenario where some patients receive one of two sequences
...
```{r }
arm1.mat <- matrix(c("adamtx", "etnmtx", "ifxmtx", "etnmtx", "adamtx", "ifxmtx"), 
...
<ul class="pager">
  ...
  <li class="next"><a href="03-structure.html">Next</a></li>
</ul>
</div>
