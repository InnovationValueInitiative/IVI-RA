...
Information related to treatment costs are stored in the list `iviRA::treat.cost`. Data related to the costs of treatment for each medication are in the data table `iviRA::treat.cost$cost`.
...
Costs for individual therapeutic agents are used to calculate costs for the (combination) treatments in a treatment sequence by using the lookup data table `iviRA::treat.cost$lookup`.

```{r }
iviRA::treat.cost$lookup
```
