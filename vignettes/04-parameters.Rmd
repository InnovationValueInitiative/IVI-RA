...
Parameters needed to run the simulation come preloaded with the package. To view the loaded data use the command `data(package = "iviRA")`. The parameters can be separated into 8 main categories: treatment effects estimated from a network meta-analysis (NMA); mappings between various measured of treatment response, disease activivity, and/or function; long-term progression of disease, treatment duration, serious infections, mortality, utility, and costs. 

### NMA for ACR response, change in DAS28, and change in HAQ
The effect of treatment on ACR response at 6 months, change in DAS28 at 6 months, and the change in the HAQ score at 6 months are estimated using a NMA. Default NMA parameter estimates come preloaded with the package and are based on results from an NMA conduced by IVI using a Bayesian random effects approach.

```{r }
names(iviRA::therapy.pars$nma.acr.naive)
names(iviRA::therapy.pars$nma.das28.naive)
names(iviRA::therapy.pars$nma.haq.naive)
```

Each object is a list containing information on the mean of the NMA and the sample variance-covariance matrix. In the PSA, we use a multivariate normal distribution to sample parameters estimated using a NMA. The advantage of this approach is that we do not have to load the entire posterior distribution of the parameters with the package. The justification is the Bayesian central limit theorem, which states that under large samples, the posterior distribution will approach a multivariate normal distribution.  

### Mapping between ACR response, EULAR response, disease activity, and HAQ
There are various mapping between treatment response measures, disease activity, and functional status that can be used within the IVI-RA model. The exact mapping utilized for a given model run will depend on the model structure select. Default parameter estimates for these mappings come preloaded with the `iviRA` package.
...
```{r }
iviRA::therapy.pars$haq.lprog
```

The constant anuual rate of HAQ progression is also adjusted by age based on differences between overall and age specific rates as reported by Michaud et al. (2011).

```{r }
iviRA::haq.lprog.age
```

The LCGM is based on the analysis by [Norton et al.](http://www.sciencedirect.com/science/article/pii/S0049017214000730). The parameters from the LCGM are contained in the list `iviRA::haq.lcgm.pars`. The coefficient estimates are stored in `iviRA::haq.lcgm.pars$coef`. 

```{r }
iviRA::haq.lcgm.pars$coef
```

The coefficient vectors `beta1`, `beta2`, `beta3`, and `beta4`, contain coefficients from a third order polynomial regression predicting the growth of the HAQ score over time for members of class 1, 2, 3, and 4, respectively; `delta2`, `delta3`, and `delta4` are coefficient vectors containing predictors of membership in classes 2-4 with class 1 being the reference group. A variance-covariance matrix, `iviRA::haq.lcgm.pars$vcov`, is used to sample parameters from a multivariate normal distribution. The indices of the parameters of the variance-covariance matrix correspond to the row numbers in `iviRA::haq.lcgm.pars$coef`.
...
Point estimates are stored in the element `est` and the variance-covaraince matrix is stored in the element `vcov`. 
...
The parametric distributions are characterized according to a location parameter governing the mean of the distribution. Models can also contain ancillarly parameters which describe the shape, variance, or higher moments of a distribution. The location and ancillary parameters associated with each distribution are shown in the table below.   
...
The `iviRA` package currently only allows for models in which the location parameter, but not the ancillary parameters, depends on covariates. The indies of the location parameter, first ancillarly parameter, and second ancillary parameter in the `est` and `vcov` objects are contained in the `loc.index`, `anc1.index`, and `anc2.index` objects, respectively. For example, in a regression model with no covariates, the location parameter is stored `r iviRA::ttd.all$weibull$loc.index` and the index of the first ancillary parameter is `r iviRA::ttd.all$weibull$anc1.index`.  
...
### Mortality
Baseline mortality for patients without RA are based on gender-specific US lifetables. More specifically, we use the probability of mortality, `qx`, between ages $x$ and $x+1$.
...
Point estimates (i.e., log odds ratios) and standard errors associated with each variable in the Wailoo model are available in the `iviRA::utility.wailoo.pars` object.

```{r }
iviRA::utility.wailoo.pars
```

The information needed to sample from the mixture model are stored in the list `iviRA::utility.mixture.pars`. The setup is the same as with the LCGM in that the list contains a table of coefficient estimates as well as the correspoding variance covariance matrix. 

```{r }
names(iviRA::utility.mixture.pars)
iviRA::utility.mixture.pars$coef
```
...
Drug acquisition and administration costs are separated into costs associated with the first 6 months of treatment and costs after the first 6 months. Costs are a function of the FDA recommended dose and frequency of administration, strength and dosage form, the wholesale acquisition cost (WAC), and infusion. If dosing is weight based, then the dose is calculated by multipling the dose value (e.g., `init_dose_val` and `ann_dose_val`) by each patients weight in kilograms; otherwise the dose is simply equal to the dose value. For example, the dose for infliximab during the first six months is equal to 3 multiplied by the weight of the patient. 

```{r }
library(knitr)
kable(iviRA::therapy.pars$cost)
```
...
Finally, we use estimates of productivity loss from [Wolfe et al. (2005)](http://www.jrheum.org/content/32/10/1875.short). The authors provide estimates of the annual loss in earnings associated with a 1-unit increase in HAQ, which we convert to 2017 dollars. 
...
<div>
<ul class="pager">
  <li class="previous"><a href="03-structure.html">Previous</a></li>
  <li class="next"><a href="05-ips.html">Next</a></li>
</ul>
</div>
