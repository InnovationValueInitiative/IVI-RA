...
The model can be simulated within the `iviRA` package using the `sim_iviRA` function. The simulation depends on three primary factors: (1) the population considered, (2) the values of the parameters, and (3) the selected model structures. The steps needed to run the model can consequently be summarized as follows:

1. Generate a patient population.
2. Select treatment arms
3. Choose model structures
4. Sample model parameters.
